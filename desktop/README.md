...
Electron/Discord/updater/native changes use two publication layers:

- production beta/stable releases are signed and published through the protected Vercel Blob release path
- development/internal builds are published as GitHub prereleases so the updater can be exercised before production signing/storage secrets are configured

The server always prefers a configured signed Blob manifest. If no signed stable/internal manifest exists, the development feed can fall back to the fixed HayKasa GitHub release tags (`desktop-latest` and `desktop-preview`). The fallback URL is server-owned and cannot be supplied by the renderer.
...
On `main`, Desktop CI also stamps a monotonically newer development version and publishes the installer, blockmap, `latest.yml`, and `release-manifest.json` to the `desktop-latest` GitHub prerelease. This path has no Blob-token dependency, so a missing production storage secret no longer turns normal Desktop CI red.

`Desktop Preview` publishes the same updater-capable bundle to `desktop-preview` for the internal channel and does not use the protected `desktop-release` GitHub Environment.
...
Stable releases are rejected unless the workflow runs from `main`. The manual `internal` path publishes the GitHub prerelease and does not require the protected environment. `beta` and `stable` use the signed release job and require the `desktop-release` environment.
...
When those are not configured, production can read the fixed GitHub prerelease fallback generated by Desktop CI. The signed Blob feed automatically takes precedence once configured.
