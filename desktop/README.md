...
Electron/Discord/updater/native changes use two paths:

- production beta/stable releases are Authenticode-signed and published through the protected Vercel Blob release path
- development/internal builds are CI artifacts for installation and smoke testing; they are not an end-user auto-update feed

The runtime updater only consumes public Vercel Blob artifacts. The source repository is private, so GitHub Releases and Actions artifacts are intentionally not used as a client update source.

The installed app:

- checks the server-owned release manifest first
...
The update endpoint is compiled into the desktop app. A remotely loaded renderer cannot choose an arbitrary executable URL.
...
On `main`, Desktop CI stamps a newer development version and uploads the installer, blockmap, and `latest.yml` as short-lived workflow artifacts. It never publishes those unsigned artifacts into the production updater feed.

`Desktop Preview` produces the same internal smoke-test artifacts without using the protected `desktop-release` GitHub Environment.
...
Stable releases are rejected unless the workflow runs from `main`. The manual `internal` path builds an unsigned test artifact only. `beta` and `stable` use the signed release job, require the `desktop-release` environment, and validate that the requested release version is newer than the currently published version before packaging.

Required GitHub `desktop-release` environment secrets for signed beta/stable publication:
...
The web deployment that supports Desktop v1 needs the desktop endpoints. Signed production publication additionally uses:
...
When those are not configured, the stable manifest remains unpublished and automatic native updates stay disabled instead of falling back to a private or unsigned source.
...
5. Build `internal` and test installation/auth/Discord/reboot behavior on a real Windows machine.
6. Publish signed `beta` to Blob and verify the full updater flow.
7. Publish signed `stable` with a low rollout percentage.
...
3. Do not present unsigned GitHub development builds as signed production releases.
