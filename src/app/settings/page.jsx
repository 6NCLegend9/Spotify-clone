  ...
  return (
    <main className="page text-white">
      ...
      <section className="grid gap-8 lg:grid-cols-2 mb-8">
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5 sm:p-7"><h2 className="mb-2 text-xl font-semibold">Taste & privacy</h2><Toggle label="Allow explicit content" checked={settings.explicitContent} onChange={(value) => set("explicitContent", value)} /><Toggle label="Private session" checked={settings.privateSession} onChange={(value) => set("privateSession", value)} /><Toggle label="Usage analytics" description="Allow Google Analytics and Simple Analytics to measure aggregate usage and service performance. This is optional and is not used for advertising." checked={settings.tailoredAds} onChange={setUsageAnalytics} /><p className="mt-4 text-xs text-[#9aa8b5]">Private session prevents new listening activity from being used for recommendations. Analytics consent applies only to this browser and can be changed here at any time.</p></div>
      </section>
