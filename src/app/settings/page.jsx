    ...
    <main className="page text-white">
      <header className="page-hero border-b border-white/10 pb-6">
        ...
        {status === "unauthenticated" ? (
          ...
          >
            {saveState === "saved" ? <FiCheck /> : <FiSave />} {saveLabel}
          </button>
        )}
      </header>
      {saveError ? (
      ...
      <section className="mb-8 rounded-xl border border-white/10 bg-white/[0.03] p-5 sm:p-7">
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          {EQ_PRESETS.map((preset, index) => <button key={preset} type="button" aria-pressed={settings.eqPreset === preset} onClick={() => set("eqPreset", preset)} className={`min-h-12 rounded-md border px-2 text-left text-xs transition ${settings.eqPreset === preset ? "border-[#00e6e6] bg-[#00e6e6]/10 text-[#00e6e6]" : "border-white/10 text-gray-300 hover:border-white/30"}`}><span className="mr-1 text-gray-500">{String(index + 1).padStart(2, "0")}</span>{preset}</button>)}
        </div>
        <div className="mt-6 grid gap-4 sm:grid-cols-5">{settings.eqBands.map((value, index) => <label key={bandLabels[index]} className="text-xs text-gray-400">{bandLabels[index]}<input type="range" min="-12" max="12" value={value} aria-valuetext={`${value > 0 ? "+" : ""}${value} dB`} onChange={(event) => dispatch(updateEqBands(settings.eqBands.map((band, bandIndex) => bandIndex === index ? Number(event.target.value) : band)))} className="mt-3 w-full accent-[#00e6e6]" /><span className="mt-1 block text-white">{value > 0 ? "+" : ""}{value} dB</span></label>)}</div>
      </section>
      ...
      <section className="grid gap-8 lg:grid-cols-2 mb-8">
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-5 sm:p-7"><h2 className="mb-2 text-xl font-semibold">Taste & privacy</h2><Toggle label="Allow explicit content" checked={settings.explicitContent} onChange={(value) => set("explicitContent", value)} /><Toggle label="Private session" checked={settings.privateSession} onChange={(value) => set("privateSession", value)} /><Toggle label="Usage analytics" description="Allow Google Analytics and Simple Analytics to measure aggregate usage and service performance. This is optional and is not used for advertising." checked={settings.tailoredAds} onChange={setUsageAnalytics} /><p className="mt-4 text-xs text-gray-500">Private session prevents new listening activity from being used for recommendations. Analytics consent applies only to this browser and can be changed here at any time.</p></div>
      </section>
